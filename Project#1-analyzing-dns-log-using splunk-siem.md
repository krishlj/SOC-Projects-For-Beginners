...
- Choose the appropriate source type for DNS logs (e.g., `dns_sample` or a custom source type if applicable).
...
- Run a search query to verify that the uploaded DNS events are visible.
  ...
  If you used a different index, replace dns_logs with the correct index name.
...
```
Replace the example value with the domain under investigation.
